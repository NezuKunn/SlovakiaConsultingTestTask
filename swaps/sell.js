...
const { PublicKey, Keypair, ComputeBudgetProgram, TransactionInstruction, TransactionMessage, VersionedTransaction, Connection } = require('@solana/web3.js');
...
async function getTokenBalances(connection_conf, walletAddress, inMint) {
    let address = new PublicKey(walletAddress.publicKey.toBase58());

    const tokenAccounts = await connection_conf.getParsedTokenAccountsByOwner(address, { programId: splToken.TOKEN_PROGRAM_ID });

    for (let tokenAccountInfo of tokenAccounts.value) {
        const tokenAccountData = tokenAccountInfo.account.data.parsed.info;

        if (tokenAccountData.mint === inMint) {

            const mintAddress = new PublicKey(tokenAccountData.mint);
            const tokenAmount = tokenAccountData.tokenAmount.uiAmount;
    
            const mintInfo = await splToken.getMint(connection_conf, mintAddress);
...
async function sell(key, rpc, mint1, mint2, amount, gas, slippage) {
    const payer = Keypair.fromSecretKey(Uint8Array.from(JSON.parse(key)));

    const connection = new Connection(rpc, 'processed');
    const connection_conf = new Connection(rpc, 'confirmed');

    let in_t = new PublicKey(mint2)
    let token = new PublicKey(mint1)
    ...
    let token_info = await getTokenBalances(connection_conf, payer, mint2)

    let desAmount = Math.floor(Number(amount) * token_info.decimals);
...
// await sell(key, rpc, inMint, outMint, amount, gas, slippage);
